...
## Порядок вторичных целей

`NearbyTargetsStaggerRequest` фиксирует событие Blunderbuss относительно позиции существ в момент попадания, но K1 не выбирает и не сортирует цели по Zones. Executor уже принимает выбранный упорядоченный набор разных вторичных целей и обрабатывает его слева направо после основной цели. Перед полноценной симуляцией нужно определить стабильный spatial-порядок либо отдельную policy выбора, если порядок способен изменить исход.
