...
Страницы 94–95 задают Reload как Exacting Dexterity Test с одной Test за action и weapon-specific целями `2–5`; отсутствие числа означает бесплатную подготовку в составе attack. Repeater даёт необязательный бонус к одному Shooting Test и требует указанную перезарядку именно после его применения. Страница 116 перечисляет ровно шесть combat actions, а страница 117 относит применение Skill в бою к Improvise. `ranged_weapon_profiles.py` фиксирует reload-поля всех 13 строк страницы 95 и три triggers. `reload_models.py` и `reload_resolution.py` реализуют action-cost ветвь как profile-bound non-attack Skill Improvise: один активный reload cycle, один Test/receipt за вызов и loaded только при Exacting completion; initial loaded state может не иметь предшествующей истории. `ranged_weapon_attack_models.py` и соответствующий resolver предоставляют единый free/reloadable Shooting entry point: free-state сохраняется, обычное оружие после hit/miss открывает уникальный unloaded cycle, Repeater делает это только при точном optional bonus, а kernel/receipt исполняются один раз. `hidden_ranged_weapon_attack` modules связывают тот же переход с Move Quietly opportunity без второго Attack. Ammunition и остальные поля полного weapon catalog пока не исполняются.
