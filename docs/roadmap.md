...
R1 завершён; round/side/turn state, базовый action budget, обычный Attack adapter, casting pipeline, обе ветви free move, обе фазы Run и обе дальности Charge реализованы. До M2 продолжается K1: следующий срез вводит общую Athletics-фазу Difficult Terrain с падением Prone сразу после пересечения и явным turn usage, после чего связывает её с free move/Run/Charge. Затем подключаются остальные Manoeuvre и skipped Casting consequences.
