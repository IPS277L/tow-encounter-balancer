...
R1 завершён; round/side/turn state, базовый action budget, обычный Attack adapter, normal/triggered spell Improvise pipeline, skipped Casting Test после Attack и добровольное прекращение Casting также реализованы. До M2 продолжается K1: следующий срез типизирует один раз за ход доступное free movement в Medium Range (либо Long для fast creature) как отдельную incidental-фазу поверх `SpatialBattleState`, без расхода action slot. Затем на эту границу опираются варианты Manoeuvre и skipped consequences остальных action kinds.
