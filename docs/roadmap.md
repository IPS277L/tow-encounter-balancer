...
R1 завершён, все строки Miscast Table, Casting Test accumulation, normal CAST/WAIT, первый typed spell-schema/target preflight, target-scoped Potency/execution boundary, stable Zone batch, movement gate, Zone graph/placement и первый конкретный formal spell реализованы. До M2 продолжается K1: следующий срез — typed round/side/turn state и базовый action budget (одно обычное действие, максимум два, без повтора и второй атаки), затем подключение существующих attack/casting/movement entry points.
