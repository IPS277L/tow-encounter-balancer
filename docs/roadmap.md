...
Готово ядро Test/Opposed/Attack/Staggered/Wound, специализированные эффекты всех строк Wounds Table, заменяющие Damage `ImpactSpec`, Hazard resolver, первые фазовые/multi-target `SecondaryEffectSpec`, Damage плюс Condition, executors выбранных вторичных/Zone Hazard целей, упорядоченные последствия Give Ground, Terrifying и все найденные явно именованные профильные Reactions: Monstrous Flight, Unsteady, Monstrous Regeneration и Undead Monstrosity. Source-classified психологическая иммунность undead-профилей покрывает непосредственные и отложенные боевые Condition-фазы, явно психологические Hazards и оба последствия `Curse of Cowardly Flight`; Foul Stench добавляет target-owned выбор между inventory follow-up и Condition, а Soporific Breath — общий Hazard выбранной Zone и явную замену повторного Drained на Defenceless. Spatial target discovery, inventory mutation, casting и turn orchestration остаются будущему battle loop; новые конкретные эффекты подключаются фазово без универсального языка правил.
