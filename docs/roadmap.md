...
R1 завершён; round/side/turn state и базовый action budget также реализованы. До M2 продолжается K1: следующий срез — узкое исполнение зарезервированного `Attack` action через существующий `KernelAttackRequest` с проверяемой связью slot → result, без универсального диспетчера. Затем тем же фазовым способом подключаются casting и movement entry points.
