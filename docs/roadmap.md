...
R1 завершён; round/side/turn state, базовый action budget и обычный Attack adapter также реализованы. До M2 продолжается K1: следующий срез — специализированное исполнение casting attempt как `Improvise` через существующий `CastingTestRequest → CastingTestResult`, с action receipt и без обобщения всех вариантов Improvise. Затем тем же фазовым способом подключаются casting decision и movement entry points.
