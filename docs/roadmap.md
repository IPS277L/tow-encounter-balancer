...
Готово ядро Test/Opposed/Attack/Staggered/Wound, специализированные эффекты всех строк Wounds Table, заменяющие Damage `ImpactSpec`, Hazard resolver, первые фазовые/multi-target `SecondaryEffectSpec` и единый `ResolutionResult`. Следующий слой принимает уже выбранный упорядоченный набор вторичных целей; Talents и NPC Abilities наращиваются поверх тех же фаз.
