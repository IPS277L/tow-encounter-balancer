...
R1 завершён; round/side/turn state, базовый action budget, обычный Attack adapter, casting pipeline, общий interrupted-Casting consequence, обе ветви free move, standalone Difficult Terrain с free-move/base-Run/Medium-Charge consumers, обе фазы Run, обе дальности Charge, Move Carefully, Move Quietly, Aim, Help, Recover и Skill Improvise реализованы. До M2 продолжается K1: следующий срез применит successful Skill-Improvise Condition request к актуальному target state через общий immunity-aware reducer, затем подключит конкретные Ability Improvise и оставшиеся application boundaries.
