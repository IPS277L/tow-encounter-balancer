...
R1 завершён; round/side/turn state, базовый action budget, обычный Attack adapter, normal/triggered spell Improvise pipeline, skipped Casting Test после Attack, добровольное прекращение Casting и обе ветви free move также реализованы. До M2 продолжается K1: следующий срез подключает базовый `ManoeuvreKind.RUN` к зарезервированному action slot и spatial state — Normal/Fast actor перемещается на одну дополнительную Zone сверх free move, Slow/Burdened блокируются. Optional Athletics для второй extra Zone остаётся следующей явной post-movement фазой. Затем подключаются остальные Manoeuvre и skipped Casting consequences.
