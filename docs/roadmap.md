...
R1 завершён; round/side/turn state, базовый action budget, обычный Attack adapter, normal/triggered spell Improvise pipeline, skipped Casting Test после Attack, добровольное прекращение Casting, обе ветви free move и обе фазы Run также реализованы. До M2 продолжается K1: следующий срез начинает `ManoeuvreKind.CHARGE` с typed preflight базовой цели в Medium Range, movement-to-Close context и `+1d` только к последующей Melee Attack; Long Range Athletics остаётся следующей фазой. Затем подключаются остальные Manoeuvre и skipped Casting consequences.
