...
R1 завершён; round/side/turn state, базовый action budget, обычный Attack adapter, normal/triggered spell Improvise pipeline и skipped Casting Test после Attack также реализованы. До M2 продолжается K1: следующий срез типизирует добровольное преждевременное прекращение активного Casting с книжным обязательным Miscast на текущем пуле, включая корректный no-dice исход и очистку Casting snapshot без вымышленного броска. Затем тем же фазовым способом подключаются movement entry points и skipped consequences остальных action kinds.
