...
Как и остальные pure reducers, точный повтор одного неизменного request воспроизводит тот же результат; глобальную дедупликацию request ID должен обеспечить будущий battle aggregate. Bypass Move Carefully остаётся следующей terrain-aware границей.
