...
Два теста охватывают hit/miss × нулевой/положительный Aim и отдельный failure Recover. Unsafe Recover отклоняется до RNG; failure сохраняет Broken и завершает action slot. Spatial/Condition/hidden consumption/history snapshots передаются без ручного снятия Broken или восстановления opportunity. От первой скрытой позиции не появляется used-position запись после Give Ground; только завершённый выстрел регистрирует вторую позицию, расходует Aim и hidden opportunity и разряжает Crossbow с новым reload cycle. Повтор и реактивация отклоняются, входные состояния неизменны. Производственные API и правила не менялись.
