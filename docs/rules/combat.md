...
`RestAndRecoveryEndeavourRequest` представляет одну выбранную Endeavour в конкретном downtime и проводит книжную Endurance Test. Провал не создаёт healing source. Успех создаёт `RestAndRecoveryEndeavourResult`, разрешает отдельной application-фазе исцелить ровно одну выбранную treated/resolved Wound и одновременно возвращает typed `FesteringWoundsRecoveryRequest` для всех Festering Wounds. Строки `16–19` проходят напрямую; строки `20–23` требуют успешный ordinary `DowntimeSurgeryResult` для той же Wound/target/state/downtime. Healing reducer переиспользует общий transition, сохраняет permanent effects и однократно погашает surgery proof перед Endeavour ID. Festering follow-up пока не мутирует отсутствующее campaign infection state.

Ordinary surgery выполняет отдельную Dexterity Test и требует Anatomy Lore, operating theatre, specialist tools, time и recovery supports. Успех создаёт proof без изменения injury state. Провал не выбирает книжный риск скрытой policy, а возвращает GM-owned `SurgeryFailureRiskRequest` с permanent disfigurement/death. Combat Surgeon, paid NPC service и применение failure outcome не входят в эту downtime boundary.
