...
`CatchYourBreathHealingRequest` является следующей послебоевой фазой и принимает точный завершённый `EndBattleWoundTreatmentResult`. Reducer сверяет каждую историческую запись с Wounds Table и за один переход полностью исцеляет все ещё активные Wounds категории `CATCH_YOUR_BREATH`, не затрагивая более долгие категории. Вылеченная Wound остаётся в журнале с прежним sequence, но больше не считается активной; все её непостоянные effects заканчиваются независимо от их прежнего duration, а `PERMANENT` сохраняются. Общая Condition удаляется только при explicit отсутствии другого источника. Treatment result ID погашается один раз; эта фаза не создаёт action receipt, Test или RNG.
