...
Ability Improvise подключается только специализированными контрактами конкретных способностей. Первый такой composite — Troll Vomit: stable approach равен `RULE-NPC-019:troll-vomit`, наличие этой Ability передаётся отдельным actor snapshot, а выбранная цель должна быть врагом, находиться на Close Range и уже иметь Staggered. Это Hazard action, а не Attack action: executor выполняет Endurance Hazard (3) и общий Wound pipeline, затем завершает slot одним receipt. Зарезервированный, но неисполненный Ability Improvise теперь блокирует окончание хода наравне с Skill и spell.
...
Альтернативные `TREAT_WOUND` и `REMOVE_CONDITION` не получают ни одной standard-выгоды. Treatment требует выбранную untreated Wound, подходящие trappings и либо Recall Test, либо auto-success от явно релевантного Lore; успех создаёт `RecoverWoundTreatmentApplicationRequest`, но ещё не мутирует source-aware Wound effects. Condition alternative выполняет один явно релевантный Test и при успехе снимает ровно одну не-Staggered/не-Prone Condition; external fact `underlying_cause_allows_removal` запрещает бросок, пока действует источник Condition. Failure обеих Test-ветвей всё равно завершает action. Defenceless закрывает Recover, а Broken может Recover только в Zone без врага. End-of-battle automatic Recover остаётся отдельной будущей границей.
