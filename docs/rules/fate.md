...
Прямой `CharacterWoundLifecycleRollRequest → RollResult → CompletionRequest → CompletionResult` закрепляет timing книги. Первая фаза выполняет Wounds Table и останавливается с неразрешённым effect. Во второй фазе caller после просмотра результата либо передаёт actor-owned `FateNearMissBurnRequest`, и composite атомарно выполняет burn/application без daily receipt, либо закрывает окно, регистрирует Wound за день и только затем исполняет effect. Обычная не-Fate отмена также завершается без регистрации/effect. Roll ID и Near Miss effect ID погашаются раздельно; stale injury/day snapshots и replay отклоняются. Подключение pending-фазы к kernel/Hazard composites остаётся следующим orchestration-срезом.
