...
Unsteady Giant сохраняет более узкий `ReactorZoneHazardRequest` и `ReactorZoneHazardResolutionRequest`: эта обёртка дополнительно требует включить самого реагирующего Giant, после чего делегирует общему Zone executor. Soporific Breath использует общий Zone Hazard request внутри специализированного Ability-action executor: выбранная Zone находится в Medium Range, её ordered targets должны точно совпасть со spatial placements и не обязаны содержать Forest Dragon.

Troll Vomit (GM Guide, страница 182) является одиночной Endurance-экспозицией Hazard (3), а Swamp Breath Troll Hag (страница 183) — тем же Hazard (3) для каждого существа выбранной Zone. Оба источника используют стандартную Wound по shortfall без failure Conditions и имеют разные специализированные Ability-action executors. Troll Vomit проверяет вражескую Staggered-цель на Close Range. Swamp Breath запрещён Staggered действующей, принимает Zone на Medium Range и доказывает полный ordered состав её placements без фильтра по стороне. Оба composite завершают slot только после полного Hazard result.
...
Классификация относится к источнику целиком и не выводится из Skill проверки либо будущего Condition. Это подтверждает профиль Vampire (Gamemaster’s Guide, страница 170): он иммунен к психологическим эффектам, но солнечный свет остаётся Hazard (2), накладывающим Ablaze и сопротивляемым через Willpower. Поэтому один лишь `Willpower` не превращает Hazard в психологический.
