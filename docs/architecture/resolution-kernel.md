...
Hazard может явно задать `RepeatedConditionReplacement`. Resolver сначала завершает Wound и её непосредственный эффект, затем на актуальном состоянии решает, применять исходное failure Condition или книжную замену. Эта узкая policy покрывает Soporific Breath без универсального языка эффектов. Специализированный action adapter проверяет доступность действия, Medium Range и точный ordered spatial snapshot Zone до вызова общего Hazard reducer.

Фабрики конкретных NPC Abilities нормализуют только последствия после успешного объявления действия. Поэтому Troll Vomit возвращает одиночную `HazardExposureRequest`, а Swamp/Soporific Breath — разные `ZoneHazardRequest`. Специализированные action adapters над kernel проверяют расход Ability Improvise slot, actor/Ability/Condition/range snapshots и актуальный spatial selection: Vomit требует одну вражескую Staggered-цель на Close Range, а оба Breath — полный ordered состав выбранной Zone на Medium Range без фильтра по стороне. Soporific adapter принимает подтверждённую inherited Ability Wood Elf как обычную запись actor snapshot, но не вычисляет mount relationship. Перенос итоговых injury states в общий battle aggregate остаётся внешним.
...
Move Quietly является отдельным composite над общим `OpposedTestRequest` и явным hiding choice. Стабильный snapshot наблюдателей содержит внешний `vigilance_priority`, поскольку книга не определяет формулу сравнения бдительности; kernel выбирает максимум со стабильным tie-order и проверяет, что opponent Test принадлежит выбранному Awareness. Route-вариант предварительно валидирует условный `FreeMovementRequest`, а same-Zone вариант способен потратить usage без вымышленного пересечения Zone. Оба применяются только при победе initiator и явных cover/hiding-position facts. Failure и decline оставляют spatial state неизменным, но завершают action slot. Hidden branch создаёт узкий one-next-unopposed-attack follow-up. Отдельный adapter проверяет source opportunity, неизменившийся actor placement/hiding position, current unaware target и готовую unopposed Attack, затем вызывает обычный Attack executor и атомарно добавляет opportunity ID в immutable consumption chain. Другой action или утративший применимость контекст дают закрытый loss reason без Attack и также погашают ID; глобальная event-шина не используется.

Recover treatment следует той же двухфазной модели без второго action receipt. Успешный Recall либо auto-Lore создаёт `RecoverWoundTreatmentApplicationRequest`; `RecoverWoundTreatmentResolutionRequest` затем обязан получить полный успешный `RecoverActionExecutionResult`, точно тот же `CharacterInjuryState` и непросроченный ordered consumption snapshot. Чистый reducer отмечает treated только выбранный `WoundRecord`, удаляет только его `ActiveWoundEffect` со сроком `UNTIL_TREATED` и сохраняет permanent/`UNTIL_HEALED`/чужие wound effects. Поскольку общий `ConditionState` не хранит все источники, каждая реально присутствующая Condition среди удаляемых эффектов требует explicit `RecoverWoundConditionSourceSnapshot`: значение `false` разрешено только при отсутствии известного оставшегося wound-effect source. Application ID добавляется ровно один раз; stale state, failed/другой Recover и повторное потребление отклоняются до перехода.

Automatic end-battle treatment не подделывает successful action result. `EndBattleTreatmentContext` отдельно и target-scoped подтверждает battle ID, окончание боя и возможность перевести дух; `EndBattleWoundTreatmentRequest` добавляет актуальный injury snapshot и обязательный trappings gate. Общий treatment transition получает ordered sequences всех untreated Wounds, поэтому одним неизменяемым переходом помечает их treated и снимает их `UNTIL_TREATED` effects, используя ту же проверку Condition sources. Уже treated Wounds и остальные durations не меняются. Context ID погашается один раз во внешней ordered chain; отсутствие ран, смерть, незавершённый бой, отсутствие передышки/trappings и повтор закрываются до reducer. Action slot, Test и RNG в этом контракте отсутствуют.

Healing является отдельной фазой, а не продолжением treatment mutation. `WoundRecord.healed` отличает полностью вылеченную рану от лишь обработанной; healed подразумевает treated и resolved effects. Запись остаётся в ordered history, поэтому новый Wound получает следующий монотонный sequence, но healed-запись не считается активной и не добавляет куб к будущей таблице. `EndEncounterHealingOpportunity` хранит encounter/target, точный injury snapshot и два внешних lifecycle-факта: encounter завершён, непосредственная опасность прошла. Optional `EndBattleWoundTreatmentResult` доказывает предшествующий automatic treatment и обязан совпасть по battle/target/state; если нужные Wounds были treated раньше, opportunity остаётся полноценным самостоятельным источником. `CatchYourBreathHealingRequest` принимает этот opportunity, полный ordered список всех treated/resolved, ещё не healed Wounds соответствующей категории и Condition-source snapshots. Необработанные либо ещё не разрешённые записи не входят в ready set и не блокируют другие готовые Wounds. Resolver повторно сверяет `entry_id`/`table_total` с Wounds Table, снимает все непостоянные effects выбранных ран, сохраняет `PERMANENT` и чужие effects, а общую Condition удаляет лишь без другого источника. Opportunity ID погашается один раз; action receipt, Test и RNG не создаются. Более тяжёлые healing tiers используют отдельные lifecycle sources, описанные ниже. См. [`ADR-0005`](../decisions/ADR-0005-wound-healing-lifecycle.md).

`NightsRespiteHealingOpportunity` является отдельным rest-source над тем же точным injury snapshot. Он требует explicit `took_it_easy`, завершённую раннюю ночь и наступившее утро, но не хранит число часов: книга не задаёт жёсткую длительность. `NightsRespiteHealingRequest` переиспользует общий immutable transition и Condition-source validation, а resolver меняет полный ready set только для `HealingRequirement.NIGHTS_REST` (`8–15`). Rest-opportunity ID продолжает ту же ordered consumed-source chain; действие, RNG и Endurance Test отсутствуют. Optional раннее healing по Endurance остаётся будущей GM-policy boundary, а не неявной частью обычного ночного отдыха.

`RestAndRecoveryEndeavourRequest → RestAndRecoveryEndeavourResult` является первым узким downtime executor: он сохраняет downtime/target/exact injury snapshot и проводит общий Basic Endurance Test через внедрённый RNG. Успех служит source для отдельного `RestAndRecoveryHealingRequest`, который выбирает ровно одну ready Wound, применяет общий immutable healing transition и погашает Endeavour ID. В отличие от двух более лёгких tiers, reducer не требует полного ready set. Тот же успешный Endeavour result несёт `FesteringWoundsRecoveryRequest`. Его отдельный target-bound consumer погашает follow-up ID, очищает все записи `FesteringWoundState` и не затрагивает ordinary injury state. Активный Festering state явно преобразуется в один source-aware `WoundDiceModifier` с величиной по числу записей; так future Wound pipeline получает книжные дополнительные untreated dice без дублирования записей в `CharacterInjuryState`.

`DailyWoundRegistrationRequest → Result` принимает только фактически принятую table-roll либо fixed character Wound, привязывает её к day/target и сохраняет стабильную identity исходной записи до treatment/effect resolution. Near Miss, повтор source/sequence, другая цель и закрытый день отклоняются. `EndOfDayInfectionRequest → Result` требует непустой журнал, актуальную injury history и отдельный `FesteringWoundState`, исполняет Endurance Test через общий RNG и сравнивает successes с дневным числом Wounds, а не с обычным Basic-success threshold. Недобор добавляет ровно одну Festering Wound независимо от shortfall; success сохраняет state. Обе ветви закрывают day-state ID один раз. Treatment/healing разрешены между регистрацией и проверкой, поскольку stable identity намеренно исключает lifecycle flags. Anatomy Recall станет отдельным proof/bypass producer и не будет синтетически создавать `TestResult`.

`DowntimeSurgeryRequest → DowntimeSurgeryResult` является prerequisite boundary для `SURGERY_AND_RECOVERY` (`20–23`). Request связывает qualified surgeon, одну treated/resolved Wound, exact target/state/downtime, все книжные resource facts и Basic Dexterity Test. Resolver сверяет Wounds Table до RNG. Success сохраняет неизменённый injury state и служит proof; failure вместо вымышленной мутации создаёт GM-owned `SurgeryFailureRiskRequest` с двумя книжными рисками. `RestAndRecoveryHealingRequest` принимает proof только для той же Wound/target/state/downtime, добавляет его Rule trace и погашает surgery ID перед Endeavour ID. Обычная рана `16–19` лишний proof отвергает.

Первая ветвь `CombatSurgeonTreatmentRequest → CombatSurgeonTreatmentResult` является triggered post-treatment boundary без нового action receipt. Она принимает полный successful `RecoverWoundTreatmentResolutionResult`, точный post-treatment injury snapshot и отдельную Recall Test. На success immutable `CombatSurgeonEffectSuppression` связывает battle/surgeon/target/stable Wound record, оба treatment source ID, Test ID и полный ordered набор effects этой Wound со сроком `UNTIL_HEALED`; на failure suppression отсутствует. В обоих исходах treatment-result ID погашается один раз. Injury state, Conditions, Wound history и effect provenance остаются неизменными.

`CombatSurgeonSuppressionRegistrationRequest → Result` однократно добавляет только successful canonical suppression в aggregate того же battle; повтор suppression/source и вторая регистрация одной target/Wound пары отклоняются. `CombatSurgeonEffectiveEffectsRequest → Result` принимает aggregate и актуальный target injury snapshot, сверяет stable Wound identity и точный ordered набор её активных `UNTIL_HEALED` effects, после чего возвращает только вычисляемые `effective_wound_effects`/`effective_conditions`. Канонический `CharacterInjuryState` не мутируется. Общий `WoundConditionSourceSnapshot` сохраняет Condition при известном другом wound-effect либо явно подтверждённом внешнем источнике. После healing зарегистрированная suppression остаётся в audit aggregate, но больше не входит в active/effective view; другой battle, target или изменённая identity отклоняются.

Общий `ExactingTestProgress` хранит ordered Basic contributions вместо изменяемого счётчика: successes являются вычисляемой суммой, включая нулевые trace-вклады и возможное превышение цели. Reducer запрещает вклад в completed progress и повтор request/Test IDs, но намеренно не моделирует цену: её обязан доказать специализированный consumer. `CombatSurgeonBattleSurgeryActionRequest → Result` является первым таким consumer. Он принимает только matching non-attack Ability Improvise slot, одну treated/resolved surgery-category Wound, exact battle/surgeon/target/injury context, Talent ownership, Dexterity Test, tools и recovery supports; operating theatre отсутствует по Talent, а action receipt является ценой каждой Test. Progress требует 8 successes и допускает вклад только того же surgeon. Нулевая Test сохраняет progress и создаёт существующий GM-owned failure-risk request; completed Test возвращает proof без healing или injury mutation.

`RestAndRecoveryHealingRequest` принимает completed `CombatSurgeonBattleSurgeryProof` как альтернативу ordinary surgery result только для `SURGERY_AND_RECOVERY`. Battle proof не обязан равняться полному более позднему downtime injury snapshot: consumer сверяет target, sequence и стабильную identity записи (`entry_id`, `table_total`, `roll_values`, `origin`), а актуальный state остаётся точно привязан к Endeavour и требует ещё не healed treated/resolved Wound. Proof ID погашается один раз перед Endeavour ID. Ветка `DowntimeSurgeryResult` сохраняет прежнюю строгую проверку same state/downtime, а строки `16–19` по-прежнему отвергают любой surgery proof.
