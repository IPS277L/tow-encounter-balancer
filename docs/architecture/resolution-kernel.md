...
Healing является отдельной фазой, а не продолжением treatment mutation. `WoundRecord.healed` отличает полностью вылеченную рану от лишь обработанной; healed подразумевает treated и resolved effects. Запись остаётся в ordered history, поэтому новый Wound получает следующий монотонный sequence, но healed-запись не считается активной и не добавляет куб к будущей таблице. `CatchYourBreathHealingRequest` принимает точный `EndBattleWoundTreatmentResult`, post-treatment injury snapshot, полный ordered список ещё не healed Wounds соответствующей категории и Condition-source snapshots. Resolver повторно сверяет `entry_id`/`table_total` с Wounds Table, снимает все непостоянные effects выбранных ран, сохраняет `PERMANENT` и чужие effects, а общую Condition удаляет лишь без другого источника. Treatment result ID погашается один раз; action receipt, Test и RNG не создаются. Остальные healing tiers и возможность открыть healing window без нового treatment result требуют следующих lifecycle sources. См. [`ADR-0005`](../decisions/ADR-0005-wound-healing-lifecycle.md).
