...
Miscast Pool разрешается ещё одной чистой фазой. `WizardMagicState` содержит только изменяемый счётчик, `wizard_level` передаётся отдельно как характеристика определения мага. Применение increase не очищает сработавший пул: результат создаёт `MiscastRollRequest` на все кубы и оставляет `state.miscast_dice > wizard_level`, из-за чего повторное увеличение до разрешения Miscast отклоняется. Будущие roll/table/effect фазы обязаны очистить пул только после фактического результата эффекта.
