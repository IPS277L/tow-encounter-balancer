...
Основная и вторичные цели используют один `StaggerImpactRequest → StaggerImpactResult`, поэтому repeated-Staggered, injury policy, Near Miss и Wound effects не дублируются между kernel и multi-target слоем. Тот же контракт принимает `ConditionAfterGiveGroundSpec`: при выбранном перемещении follow-ups строго упорядочены как Give Ground, затем Condition. Каждый вторичный результат остаётся связан с `target_id`; применённые secondary rules записываются в trace. Damage с дополнительным Condition и уникальные профильные Reactions ещё требуют secondary/named resolvers.
