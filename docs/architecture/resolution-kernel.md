...
Первая ветвь `CombatSurgeonTreatmentRequest → CombatSurgeonTreatmentResult` является triggered post-treatment boundary без нового action receipt. Она принимает полный successful `RecoverWoundTreatmentResolutionResult`, точный post-treatment injury snapshot и отдельную Recall Test. На success immutable `CombatSurgeonEffectSuppression` связывает battle/surgeon/target/Wound, оба treatment source ID, Test ID и полный ordered набор effects этой Wound со сроком `UNTIL_HEALED`; на failure suppression отсутствует. В обоих исходах treatment-result ID погашается один раз. Injury state, Conditions, Wound history и effect provenance остаются неизменными: будущий battle aggregate должен вычитать suppression при вычислении эффективных последствий только внутри совпадающего `battle_id`.
