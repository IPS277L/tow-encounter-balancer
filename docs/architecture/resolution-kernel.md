...
`ConditionOnGiveGroundOrWoundSpec` получает уже подтверждённый исход общего Stagger/injury pipeline. Give Ground создаёт упорядоченную пару movement → Condition, а Condition после Wound применяется лишь при `wound_accepted` либо ненулевом числе профильных Wounds. Поэтому Near Miss не срабатывает как Wound. Каждый вторичный результат остаётся связан с `target_id`; применённые secondary rules записываются в trace. Отдельно исполняемая `MonstrosityReactionRequest` пока не сообщает kernel свой фактический исход, поэтому Give Ground внутри такой Reaction ещё не может запустить это условие.
