...
Основная и вторичные цели используют один `StaggerImpactRequest → StaggerImpactResult`, поэтому repeated-Staggered, injury policy, Near Miss и Wound effects не дублируются между kernel и multi-target слоем. Каждый вторичный результат остаётся связан с `target_id`; применённые secondary rules записываются в trace. Damage с дополнительным Condition и уникальные профильные Reactions ещё требуют secondary/named resolvers.
