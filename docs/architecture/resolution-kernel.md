...
`ConditionImpactSpec` — первый подключённый путь: `KernelAttackRequest` передаёт иммунитеты цели, а replacement Condition проверяется до прямого reducer или специальной Staggered policy. Это намеренно ещё не глобальная event-шина. Secondary Conditions, Hazard failure Conditions и не-Condition эффекты должны явно вызывать тот же узкий resolver в своих известных фазах.
