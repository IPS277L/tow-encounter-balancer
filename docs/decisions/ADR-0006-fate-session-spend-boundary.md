...
Трата Fate на Glorious до либо после initial roll имеет проверяемую цену и создаёт proof, который принимает `Drained` preparation. Second Action использует тот же session pool и атомарно связывает spend с конкретным вторым slot, не помещая session state в scheduler. Tactical Retreat тем же pool оплачивает rearguard конкретного actor для конкретного battle/group, не подменяя pursuit. Permanent rating не смешивается с session allowance, история не переносится между actor/session, а повтор Test/slot/Retreat в одном actor state отклоняется. Initial pool имеет отдельный immutable provenance snapshot и не бросается повторно после решения. Lucky/free spends, refresh producer и burn остаются отдельными границами.
