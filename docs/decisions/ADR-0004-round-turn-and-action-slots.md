...
- Difficult Terrain traversal считать общим spatial reducer без собственного action slot. Он сохраняет полный source request, crossed state, Conditions и Test usage. Free move/base Run consumers доказывают равенство actor/round/origin/destination/path/Conditions и current snapshots, затем без нового RNG расходуют usage либо добавляют receipt; Medium Charge следует той же границе с отдельным attack context.
- Запрещать завершение хода с зарезервированным, но неисполненным обычным Attack, Run, Charge или spell Improvise. Для ещё не подключённых action kinds временно сохранять прежнюю границу резервации.
...
Срез пока не определяет Awareness-результат засады, прочие incidental actions, расход Fate, эффекты Aim/Help/Recover, terrain-aware Medium Charge, Move Carefully и остальные Manoeuvre, исполнение подготовленных Miscast follow-ups, target discovery, общий battle aggregate, окончание боя и AI выбора порядка. Обычный Attack, обе фазы Run, Medium/Long Charge, standalone Difficult Terrain traversal с free-move/base-Run consumers, normal post-Casting, triggered preparation, добровольное прекращение Casting, skipped Casting consequence после Attack и обе ветви free move уже исполняются; соответствующий slot должен завершиться до конца хода. Завершение активного хода требует зарезервировать standard action; если позднее книжный источник потребует явный pass/skip, он должен быть представлен отдельным нормативным действием или решением, а не пустым ходом.
