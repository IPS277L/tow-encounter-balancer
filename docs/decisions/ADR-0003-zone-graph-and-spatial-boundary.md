...
Spatial target discovery, стабильная сортировка целей, cover/line of sight, vertical/midair metadata, движение внутри Zone, bypass Difficult Terrain через Lore и остальные Manoeuvre этим решением не определены. Move Carefully определён только для заранее переданного terrain-aware route; смысл результата Awareness search остаётся внешнему scene/battle orchestration. Остальные границы добавляются отдельными контрактами по фактической потребности. Явный Long route доказывает только дальность и не утверждает точную геометрию пути внутри Zone.
