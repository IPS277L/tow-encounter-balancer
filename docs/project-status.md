...
- Trapped routing не наносит Wounds и не меняет captivity/campaign state, не выводит affected actor из rearguard и сохраняет отсутствие rearguard у alternative-price cover; wrong outcome/owner/order/reference/replay и forged proof/result отклоняются, точный объём цены остаётся `AMBIGUITY-014`;
...
Реализовать consumer ветви `Trapped Wounds`: принять exact `TrappedWoundCostApplicationRequest`, актуальные `CharacterInjuryState` и explicit положительное число Wounds для каждого affected PC, затем провести каждую Wound через общий двухфазный character-Wound lifecycle в group order. Не выводить число Wounds из текста книги и не смешивать capture/other state.
...
Результат: `Ran 874 tests ... OK`; отдельный K1-набор: `Ran 854 tests ... OK`; `compileall`, public-import smoke test для Retreat price, inventory, campaign-opportunity, Run For Your Lives campaign-registration, Robbed и Trapped contracts, проверка отсутствия domain→rules imports и `git diff --check` успешно завершены (только предупреждения Git о LF/CRLF).
