...
Реализовать outcome `Hunted` как typed campaign threat aggregate: принять exact registered consequence, explicit stable pursuer reference и activation-trigger reference, зарегистрировать неактивную во время lay-low угрозу и отдельную будущую границу её активации при следующем движении группы. Не вычислять длительность укрытия, обнаружение, маршрут или новый pursuit автоматически.
...
Результат: `Ran 896 tests ... OK`; отдельный K1-набор: `Ran 876 tests ... OK`; `compileall`, public-import smoke test для Retreat price, inventory, campaign-opportunity, Run For Your Lives campaign-registration, Robbed, Surrounded, Trapped routing, Trapped Wounds и Trapped Capture contracts, проверка отсутствия domain→rules imports и `git diff --check` успешно завершены (только предупреждения Git о LF/CRLF).
