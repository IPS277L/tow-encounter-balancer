...
| AUDIT-005 | Player’s Guide 1.4, 12, 110, 156–157 | DEFINITION/RULE | Exacting Test накапливает успехи нескольких Tests во времени; Casting привязывает попытку к Lore и использует последний бросок как Potency | Общий Basic-contribution progress и первый action-cost consumer Combat Surgeon реализованы в `RULE-TEST-007`; Casting сохраняет специализированный lifecycle, Opposed contributions и другие цены ещё partial | partially_resolved |
...
| AUDIT-020 | Player’s Guide 1.4, 111–112 | RULE | Fate разделяет session spends и permanent burn; Near Miss применяется после Wounds Table, Last Stand требует уже имеющуюся Wound | Добавлены `RULE-FATE-001..003`; session spends, permanent rating transition, три typed burn-effect boundary, exact Near Miss consumer и direct post-roll lifecycle реализованы; два narrative effect и kernel/Hazard integration остаются orchestration/policy-задачей | recorded |
| AUDIT-021 | Player’s Guide 1.4, 112–120 | RULE | Книжный battle loop использует side initiative, максимум 2 разных actions, graph-like Zones, Speed, four Manoeuvres и campaign Retreat table | Zone graph/placement и общий Give Ground реализованы; action/turn/Speed/Manoeuvre/Retreat orchestration ещё отсутствует | partially_resolved |
...
| AUDIT-030 | Player’s Guide 1.4, 114, 152–155, 162 | LORE/RULE | Нормативный общий контракт здесь ограничен Wizard terminology и CV/Target/Range/Duration; Hexenguilde/Dhar/persecution описывают мир | Добавлен `RULE-MAGIC-005`; typed schema/preflight реализован для первого spell, spatial discovery остаётся внешним; setting claims не стали modifiers/corruption rules | partially_resolved |
